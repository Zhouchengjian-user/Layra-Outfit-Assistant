    ...
    <main className={`site-shell ${loading ? "is-thinking" : ""}`}>
      ...
      <section className="studio-surface">
        ...
        {tab === "wardrobe" && <div className="screen wardrobe-screen">
          ...
          <button className="starter-banner" onClick={openStarterPicker} disabled={starterLoading}><span>⚡</span><div><b>{activeCloset === "own" ? "没有自己的衣服？先用虚拟衣柜体验" : `当前：${activeCloset === "female" ? "女生" : "男生"}虚拟衣柜`}</b><small>{activeCloset === "own" ? `一键放进 12 件示例单品，先体验推荐和试穿；之后上传了自己的衣服就能切回（现有 ${ownGarmentCount} 件）` : `这里只放示例单品 ${activeItems.length} 件，不会混入你的衣服；切回我的衣柜（${ownGarmentCount} 件）随时可用`}</small></div><i>切换 →</i></button>
          ...
          </div> : <section className="wardrobe-empty"><div><span>＋</span></div><h3>{closetFilter === "全部" ? "衣柜还是空的" : closetFilter === "清洗中" ? "没有清洗中的衣服" : `还没有${closetFilter}`}</h3><p>{closetFilter === "全部" ? "先上传一件常穿的衣服。易搭会自动抠掉背景、识别标签，你只需要确认一下；不想现在上传，也可以先用虚拟衣柜体验。" : closetFilter === "清洗中" ? "点衣服卡片上的「标记清洗」，它就会出现在这里。" : "可以切换到全部，或上传一件新的单品。"}</p><div className="wardrobe-empty-actions"><button onClick={() => openUploadPicker("replace")}>上传第一件衣服</button>{closetFilter === "全部" && <button className="starter-cta-desktop" onClick={openStarterPicker} disabled={starterLoading}>{starterLoading ? "正在准备…" : "⚡ 先用虚拟衣柜体验"}</button>}</div></section>}
