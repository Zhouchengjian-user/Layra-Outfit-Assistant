...
type WardrobeItem = {
  ...
  season: string; style: string; status: "available" | "washing"; createdAt: number; imageUrl: string; aiTags: GarmentAITags & { starterGender?: "女" | "男" }; tagVersion: number;
};
...
function YidaApp() {
  ...
  const activateStarterWardrobe = async (genderOverride?: "女" | "男") => {
    ...
    notify(targetGender === "男" ? "正在准备男生虚拟衣柜…" : "正在准备女生虚拟衣柜…");
    try {
  ...
  const mobileHome = (
    <div className="screen home-screen mobile-home">
      ...
      {!activeItems.length && !starterLoading && <button className="starter-cta-mobile" onClick={openStarterPicker}><span>⚡</span><div><b>还没上传衣服？先用虚拟衣柜体验</b><small>一键放进 12 件示例单品，先试试推荐和试穿，之后可随时换成自己的衣服</small></div><i>→</i></button>}
      {starterLoading && <section className="starter-progress-mobile"><span className="spinner" /> 正在准备虚拟衣柜，稍等几秒…</section>}
    ...
    <main className={`site-shell ${loading ? "is-thinking" : ""}`}>
      ...
      <section className="studio-surface">
        {tab === "home" && <><div className="desktop-home chat-home">
          ...
          {loading && <Thinking phase={recommendationPhase} />}
          <div className="results-anchor" />
          {showResults ? resultsBlock : <><section className="quick-start-panel"><div><span>开始推荐前</span><h3>{activeItems.length ? `${activeCloset === "own" ? "已从衣柜同步" : "已从虚拟衣柜同步"} ${activeItems.length} 件单品` : "先添加你的真实衣物"}</h3><p>{activeItems.length ? "易搭只会从这些单品里给你三个答案，不会偷偷加入陌生衣服。" : "上传、抠图并确认入柜后，才能生成真正属于你的搭配；不想上传也可以先用虚拟衣柜体验。"}</p></div><div className="quick-start-actions"><button onClick={() => setTab("wardrobe")}>{activeItems.length ? "检查衣柜" : "去添加衣物"} →</button>{!activeItems.length && activeCloset === "own" && <button className="starter-cta-desktop" onClick={openStarterPicker} disabled={starterLoading}>{starterLoading ? "正在准备…" : "⚡ 先用虚拟衣柜体验"}</button>}</div></section><ShortcutSection setTab={setTab} /></>}
        </div>{mobileHome}</>}

        {tab === "wardrobe" && <div className="screen wardrobe-screen">
          <header className="sub-header"><div><span className="micro-label">MY CLOSET</span><h2>{activeCloset === "own" ? "我的衣柜" : activeCloset === "female" ? "女生虚拟衣柜" : "男生虚拟衣柜"} <sup>{activeItems.length}</sup></h2><p>{activeCloset === "own" ? "把真实衣服整理好，之后的搭配才会真正属于你。" : "虚拟衣柜的示例单品不会和你的衣服混在一起，随时可切回自己的衣柜。"}</p></div><div className="wardrobe-header-actions"><button className="starter-round" onClick={openStarterPicker} disabled={starterLoading} title="衣柜切换">{starterLoading ? <span className="spinner" /> : "⚡"}</button><button className="round-add" onClick={() => openUploadPicker("replace")} aria-label="上传衣物">+</button></div></header>
          <button className="starter-banner" onClick={openStarterPicker} disabled={starterLoading}><span>⚡</span><div><b>{activeCloset === "own" ? (ownGarmentCount ? "衣柜切换" : "没有自己的衣服？先用虚拟衣柜体验") : `当前：${activeCloset === "female" ? "女生" : "男生"}虚拟衣柜`}</b><small>{activeCloset === "own" ? (ownGarmentCount ? `你的衣柜 ${ownGarmentCount} 件，也可以在虚拟衣柜（女/男各 12 件）之间切换` : "一键放进 12 件示例单品，先体验推荐和试穿；之后上传了自己的衣服就能切回") : `这里只放示例单品 ${activeItems.length} 件，不会混入你的衣服；切回我的衣柜（${ownGarmentCount} 件）随时可用`}</small></div><i>切换 →</i></button>
          ...
          {closetFilter === "全部" && <button className="upload-zone" onClick={() => openUploadPicker("replace")}><span className="upload-icon"><Icon name="camera" /></span><span><b>拍照或从相册上传衣物</b><small>自动逐件识别、抠图与整理标签，确认后加入衣柜</small><em>一张图可包含多件单品 · 单次最多 5 张</em></span><strong>开始上传 →</strong></button>}
          ...
          </div> : <section className="wardrobe-empty"><div><span>{closetFilter === "全部" ? "＋" : "○"}</span></div><h3>{closetFilter === "全部" ? "衣柜还是空的" : closetFilter === "清洗中" ? "没有清洗中的衣服" : `还没有${closetFilter}`}</h3><p>{closetFilter === "全部" ? "先上传一件常穿的衣服。易搭会自动抠掉背景、识别标签，你只需要确认一下；不想现在上传，也可以先用虚拟衣柜体验。" : closetFilter === "清洗中" ? "在衣物卡片上点「标记清洗」，它就会出现在这里。" : "这个分类下还没有单品，可以切回全部看看。"}</p>{closetFilter === "全部" ? <div className="wardrobe-empty-actions"><button onClick={() => openUploadPicker("replace")}>上传第一件衣服</button><button className="starter-cta-desktop" onClick={openStarterPicker} disabled={starterLoading}>{starterLoading ? "正在准备…" : "⚡ 先用虚拟衣柜体验"}</button></div> : <div className="wardrobe-empty-actions"><button onClick={() => setClosetFilter("全部")}>查看全部衣服</button></div>}</section>}
      ...
      {showStarterPicker && <ModalFrame onClose={() => setShowStarterPicker(false)} panelClassName="compact-modal"><button className="modal-close" onClick={() => setShowStarterPicker(false)}>×</button><span className="micro-label">虚拟衣柜</span><h3>还没有自己的衣服？</h3><p>虚拟衣柜是给你先体验用的：一键放进一套示例单品（女/男各 12 件），马上就能生成推荐和试穿图。等之后上传了自己的衣服，随时可以切回「我的衣柜」，两者不会混在一起。</p><div className="starter-gender-grid"><button className={activeCloset === "own" ? "active" : ""} onClick={() => { setShowStarterPicker(false); removeStarterWardrobe(); }}><b>{activeCloset === "own" ? "✓ 我的衣柜（当前）" : "我的衣柜"}</b><small>你自己的衣服 · 现有 {ownGarmentCount} 件</small></button><button className={activeCloset === "female" ? "active" : ""} onClick={() => { setProfile(value => ({ ...value, gender: "女" })); saveProfile({ ...profile, gender: "女" }, stylePrefs); setShowStarterPicker(false); activateStarterWardrobe("女"); }}><b>{activeCloset === "female" ? "✓ 女生虚拟衣柜（当前）" : "女生虚拟衣柜"}</b><small>12 件示例女装 · 与你的衣服分开存放</small></button><button className={activeCloset === "male" ? "active" : ""} onClick={() => { setProfile(value => ({ ...value, gender: "男" })); saveProfile({ ...profile, gender: "男" }, stylePrefs); setShowStarterPicker(false); activateStarterWardrobe("男"); }}><b>{activeCloset === "male" ? "✓ 男生虚拟衣柜（当前）" : "男生虚拟衣柜"}</b><small>12 件示例男装 · 与你的衣服分开存放</small></button></div></ModalFrame>}
